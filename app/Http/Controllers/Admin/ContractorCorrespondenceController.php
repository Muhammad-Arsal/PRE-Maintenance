...
class ContractorCorrespondenceController extends Controller
{
    ...
    public function index(Request $request, Contractor $id)
    {
        ...
        $all = $this->paginate($all, route("admin.contractors.correspondence", $contractor->id));

        $contractors = Contractor::orderBy('name', 'asc')->get();

        return view('admin.contractors.correspondence.index', compact('page', 'contractor', 'data', 'files', 'parent', 'all', 'contractors', 'filterType'));
    }

    public function saveComment(Contractor $id, Request $request)
    {
        try {
    ...
    public function editComment(Contractor $id, Request $request)
    {
    ...
    public function createFolder(Contractor $id, $parent_id, Request $request)
    {
        $validator = Validator::make($request->all(), [
            'new_folder' => 'required',
        ]);
        ...
        if ($folder) {
            ...
            return view('admin.contractors.correspondence.table', compact('data', 'files', 'contractor', 'all'));
        } else {
            return response()->json(array(
                'success' => false,
                'type'    => 1,
    ...
    public function showUploadFileForm(Contractor $id, $parent_id, Request $request)
    {
        $page['page_title'] = 'Manage Correspondence';
        $page['page_parent'] = 'Home';
        $page['page_parent_link'] = route('admin.dashboard');
        ...
        return view('admin.contractors.correspondence.uploadFiles', compact('page', 'contractor', 'parent_id'));
    }

    public function uploadFiles(Contractor $id, $parent_id, Request $request)
    {
        $contractor = $id;
        $admin_id = auth()->guard('admin')->user()->id;
    ...
    public function showTaskPage(Contractor $id, Request $request){
        $page['page_title'] = 'Manage Correspondence';
        $page['page_parent'] = 'Home';
        $page['page_parent_link'] = route('admin.dashboard');
        ...
        $contractor = $id;
        return view('admin.contractors.correspondence.createTask', compact('page', 'contractor',  'platform_users', 'taskTray', 'contractors'));
    }
    ...
    public function storeTask(Tasks $task, Request $request)
    {
        ...
        return redirect()->route('admin.contractors.correspondence',$task->contact_id)
        ->withFlashMessage('Task added successfully!')
        ->withFlashType('success');          
    }

    public function showChild(Request $request, Contractor $id, $parent_id)
    {
        $page['page_title'] = 'Manage Correspondence';
        $page['page_parent'] = 'Home';
        $page['page_parent_link'] = route('admin.dashboard');
        ...
        $all = $data->concat($files)->concat($call);
        $all = $all->sortByDesc('created_at');
        $all = $this->paginate($all, route("admin.contractors.correspondence.child", ['id' => $contractor->id, 'parent_id' => $parent_id]));

        $contractors = Contractor::orderBy('name', 'asc')->get();

        return view('admin.contractors.correspondence.index', compact('page', 'contractor', 'data', 'files', 'parent', 'all', 'filterType', 'contractors'));
    }
    ...
    public function moveFile(Contractor $id, $parent_id, Request $request)
    {
        $validator = Validator::make($request->all(), [
            'move_link' => 'required',
        ], [
    ...
    public function newCall(Request $request, Contractor $id, $parent_id)
    {
        $contractor = $id;
    ...
    public function storeMeeting(Request $request, Contractor $id, $parent_id)
    {
        $contractor = $id;
    ...
    public function add_edit_description(Request $request, $id) {
        ...
        if ($file) {
            return redirect()->route('admin.contractors.correspondence', $id)->withFlashMessage('File Description Edited Successfully')
            ->withFlashType('success');
        } else {
            return redirect()->route('admin.contractors.correspondence', $id)->withFlashMessage('Oops! Something went wrong')
