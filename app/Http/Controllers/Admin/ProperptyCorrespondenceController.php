...
class ProperptyCorrespondenceController extends Controller
{
    ...
    public function index(Request $request, Property $id)
    {
        ...
        if ($filterType === 'folder') { 
            $data = GeneralCorrespondence::where("parent_id", 0)->where('property_id', $property->id)->where('type','property')->orderBy('name')->get();
            $files = collect();
            $call = collect();
        } elseif ($filterType === 'file') { 
            $data = collect();
            $files = GeneralCorrespondenceFiles::where("parent_id", 0)->where('property_id', $property->id)->orderBy('file_name')->get();
            $call = collect();
        } elseif ($filterType == 'call') {
            $data = collect();
            $files = collect();    
            $call = GeneralCorrespondenceCall::where('property_id', $property->id)->where('type','property')->get();
        } else {
            $data = GeneralCorrespondence::where("parent_id", 0)->where('property_id', $property->id)->where('type','property')->orderBy('name')->get();
            $files = GeneralCorrespondenceFiles::where("parent_id", 0)->where('property_id', $property->id)->where('type','property')->orderBy('file_name')->get();
            $call = GeneralCorrespondenceCall::where('parent_id', 0)->where('property_id', $property->id)->where('type','property')->get();
        }

        $parent = 0;
        $all = $data->concat($files)->concat($call);
        $all = $all->sortByDesc('created_at');
        $all = $this->paginate($all, route("admin.propertys.correspondence", $property->id));

        $propertys = Property::orderBy('created_at', 'asc')->get();

        return view('admin.properties.correspondence.index', compact('page', 'property', 'data', 'files', 'parent', 'all', 'propertys', 'filterType'));
    }

    public function saveComment(Property $id, Request $request)
    {
        try {
    ...
    public function editComment(Property $id, Request $request)
    {
    ...
    public function createFolder(Property $id, $parent_id, Request $request)
    {
        $validator = Validator::make($request->all(), [
            'new_folder' => 'required',
        ]);
        ...
        $admin_id = auth()->guard('admin')->user()->id;
        $check = GeneralCorrespondence::where('parent_id', $parent_id)->where('name', $name)->where('property_id', $property->id)->where('type','property')->first();
        if (!empty($check)) {
            return response()->json(array(
                'success' => false,
                'type'    => 1,
        ...
        $this->makeUserDirectory($property->id);

        $admin_id = auth()->guard('admin')->user()->id;

        if ($parent_id == 0) {
            $directory = public_path('generalFileManager/property-' . $property->id . '/' . $name . '/');
        ...
        if ($parent_id == 0) {
            $link = "generalFileManager/property-" . $property->id . '/' . $name;
        } else {
            $parent = GeneralCorrespondence::find($parent_id);
        ...
        if ($folder) {
            $data = GeneralCorrespondence::where('parent_id', $parent_id)->where('property_id', $property->id)->where('type','property')->orderBy('name')->get();
            $files = GeneralCorrespondenceFiles::where("parent_id", $parent_id)->where('property_id', $property->id)->where('type','property')->get();
            $all = $data->merge($files);
            $all = $all->sortByDesc('created_at');

            return view('admin.properties.correspondence.table', compact('data', 'files', 'property', 'all'));
        } else {
            return response()->json(array(
                'success' => false,
                'type'    => 1,
    ...
    public function makeUserDirectory($property_id)
    {
        $admin_id = auth()->guard('admin')->user()->id;

        $directory = public_path('generalFileManager/property-' . $property_id);
    ...
    public function showUploadFileForm(Property $id, $parent_id, Request $request)
    {
        $page['page_title'] = 'Manage Correspondence';
        $page['page_parent'] = 'Home';
        $page['page_parent_link'] = route('admin.dashboard');
        ...
        return view('admin.properties.correspondence.uploadFiles', compact('page', 'property', 'parent_id'));
    }

    public function uploadFiles(Property $id, $parent_id, Request $request)
    {
        $property = $id;
        $admin_id = auth()->guard('admin')->user()->id;
        ...
        if ($request->files) {
            foreach ($request->files as $key => $file) {
                ...
                if (!empty($file)) {
                    ...
                    if ($parent_id == 0) {
                        $directory = public_path('generalFileManager/property-' . $id->id . '/');
                    } else {
                        $parent = GeneralCorrespondence::find($parent_id);
                    ...
                    $sameName = \DB::table('general_correspondence_files')->where('file_name', $filename . '.' . $ext)->where('parent_id', $parent_id)->where('property_id', $property->id)->where('type','property')->get();
                    ...
                    if ($parent_id == 0) {
                        $link = "generalFileManager/property-" . $id->id . '/' . $filename;
                        $original_link = "generalFileManager/property-" . $id->id . '/' . $file_name;
                    } else {
                        $parent = GeneralCorrespondence::find($parent_id);
    ...
    public function showChild(Request $request, Property $id, $parent_id)
    {
        $page['page_title'] = 'Manage Correspondence';
        $page['page_parent'] = 'Home';
        $page['page_parent_link'] = route('admin.dashboard');
        ...
        if ($filterType === 'folder') { 
            ...
            $data = collect();
            $files = GeneralCorrespondenceFiles::where("parent_id", $parent_id)->where('property_id', $property->id)->where('type','property')->orderBy('file_name')->get();
            $call = collect();
        } elseif ($filterType == 'call') {
            $data = collect();
            $files = collect();    
            $call = GeneralCorrespondenceCall::where('property_id', $property->id)->where('type','property')->where('parent_id', $parent_id)->get();
        } else {
            $data = GeneralCorrespondence::where("parent_id", $parent_id)->where('property_id', $property->id)->where('type','property')->orderBy('name')->get();
            $files = GeneralCorrespondenceFiles::where("parent_id", $parent_id)->where('property_id', $property->id)->where('type','property')->orderBy('file_name')->get();
            $call = GeneralCorrespondenceCall::where('property_id', $property->id)->where('type','property')->where('parent_id', $parent_id)->get();
        }
        $parent = GeneralCorrespondence::where('id', $parent_id)->where('property_id', $property->id)->where('type','property')->first();

        $all = $data->concat($files)->concat($call);
        $all = $all->sortByDesc('created_at');
        $all = $this->paginate($all, route("admin.propertys.correspondence.child", ['id' => $property->id, 'parent_id' => $parent_id]));

        $propertys = Property::orderBy('created_at', 'asc')->get();

        return view('admin.properties.correspondence.index', compact('page', 'property', 'data', 'files', 'parent', 'all', 'filterType', 'propertys'));
    }
    ...
    public function moveFile(Property $id, $parent_id, Request $request)
    {
        $validator = Validator::make($request->all(), [
            'move_link' => 'required',
        ], [
        ...
        foreach ($fileNames as $ff) {
            ...
            $admin_id = auth()->guard('admin')->user()->id;
            if ($fileType == 'file') {
                $file = \DB::table('general_correspondence_files')->where('id', $fileId)->where('property_id', $property->id)->where('type','property')->first();

                if ($move_link == '/') {
                    $saveDir = "generalFileManager/property-" . $property->id . '/' . $file->file_name;
                    $originalDir = "generalFileManager/property-" . $property->id . '/' . $file->original_name;
                } else {
                    $saveDir = "generalFileManager/property-" . $property->id . "/" . $move_link . '/' . $file->file_name;
                    $originalDir = "generalFileManager/property-" . $property->id . "/" . $move_link . '/' . $file->original_name;
                }

                if ($move_link == '/') {
                    $directory = public_path("generalFileManager/property-" . $property->id . "/" . $file->original_name);
                } else {
                    $directory = public_path("generalFileManager/property-" . $property->id . "/" . $move_link . "/" . $file->original_name);
                }

                if ($parent_id == 0) {

                    if ($move_link != '/') {
                        $getParent = "generalFileManager/property-" . $property->id . '/' . $move_link;
                        $parent = GeneralCorrespondence::where('link', "generalFileManager/property-" . $property->id . '/' . $move_link)->where('property_id', $property->id)->where('type','property')->first();
                        if (empty($parent)) {
                            return response()->json(array(
                                'success' => false,
                                'type'    => 1,
                        ...
                        $getParent = "generalFileManager/property-" . $property->id;
                    }
                } else {
                ...
                if ($move_link != '/') {
                    ...
                    $parentIdToUpdate = GeneralCorrespondence::where("link", $getParent)->where('name', $parentFolderName)->where('property_id', $property->id)->where('type','property')->first();
                    $checkNames = \DB::table("general_correspondence_files")->where('parent_id', $parentIdToUpdate->id)->where('file_name', $fileName)->where('property_id', $property->id)->where('type','property')->get();
                    ...
                    if ($count > 0) {
                        ...
                        if (rename(public_path($file->original_link), $directory)) {
                            \DB::table('general_correspondence_files')->where('id', $fileId)->where('property_id', $property->id)->where('type','property')->update([
                                'link'  => $saveDir,
    ...
    public function newCall(Request $request, Property $id, $parent_id)
    {
        $property = $id;
    ...
    public function add_edit_description(Request $request, $id) {
        ...
        if ($file) {
            return redirect()->route('admin.propertys.correspondence', $id)->withFlashMessage('File Description Edited Successfully')
            ->withFlashType('success');
        } else {
            return redirect()->route('admin.propertys.correspondence', $id)->withFlashMessage('Oops! Something went wrong')
