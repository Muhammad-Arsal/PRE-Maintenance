...
(function(window, document, $) {
    ...
    //adding new row for social configurations


     $(document).on('click', '.deleteRow', function(){
          var td_id = $(this).attr('data-row-id');
          $('#td-'+ td_id).remove();
     });
    ...
    $(document).on('click', '.addRow',() => {
       ...
       var rowType = $('.addRow').data('row-type');
       var rowCount = parseInt($('.addRow').attr('data-row'));
     
       rowCount+=1;

       if(rowType !== 0) $('#social-table .social-cols tr:last').remove();

    
        $('#social-table .social-cols tr:last').after('<tr  id="td-'+seconds+'"></tr>');
        $('#social-table .social-cols tr:last').append('<td>'+rowCount+'</td>');
        $('#social-table .social-cols tr:last').append('<td><input type="text" class="form-control" name="title[]" /></td>');
        $('#social-table .social-cols tr:last').append('<td><input type="text" class="form-control" name="url[]" /></td>');
        $('#social-table .social-cols tr:last').append('<td><input type="text" class="form-control" name="icon[]" /></td>');
        $('#social-table .social-cols tr:last').append('<td><input type="number" class="form-control" name="sort[]" /></td>');    
        $('#social-table .social-cols tr:last').append('<td><span  style="padding:0.5rem 0.75rem" data-row-id="'+seconds+'" class="deleteRow d-inline-block rounded bg-danger bg text-white"><i class="la la-trash"></i></span></td>');   
        
        if(rowType === 0)  $('#addRow').parents('tr').remove();
        
        // 
    
  
          $('#social-table .social-cols').append('<tr><td colspan="5"></td><td><span data-row="'+rowCount+'"  data-row-type="1"  style="padding:0.5rem 0.75rem" class="addRow d-inline-block rounded bg-success bg text-white"><i class="la la-plus-circle"></i></span></td>');
